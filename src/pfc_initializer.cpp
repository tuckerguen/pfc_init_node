...
void PfcInitializer::computeNeedlePose(bool multi_thread) {
	...
	if (multi_thread) {
		...
		l_matches = collect_top_matches(l_matches, left_templ.params.num_cand_pts);
		r_matches = match(r_img.image, right_templ);
		r_matches = collect_top_matches(r_matches, right_templ.params.num_cand_pts);
	}
...
void PfcInitializer::displayResults(const NeedlePose &true_pose) {
	...
	cv::Mat l_img_c;
	cv::Mat r_img_c;
	cvtColor(l_img.image, l_img_c, CV_GRAY2RGB);
	cvtColor(r_img.image, r_img_c, CV_GRAY2RGB);
	for (int i = 0; i < l_matches.size(); i++) {
		TemplateMatch match_l = l_matches.at(i);
		TemplateMatch match_r = r_matches.at(i);
		int color_inc = 255/l_matches.size();

		match_l.drawOnImage(l_img_c, cv::Scalar(i*color_inc, 255 - i*color_inc, 180));
		match_r.drawOnImage(r_img_c, cv::Scalar(i*color_inc, 255 - i*color_inc, 180));
		drawNeedleOrigin(l_img_c, match_l.origin, cv::Scalar(0, 255, 255));
		drawNeedleOrigin(r_img_c, match_r.origin, cv::Scalar(0, 255, 255));
	}
	...
	cv::imshow("left", l_img_c);
	cv::imshow("right", r_img_c);
	cv::waitKey(0);
...
vector<string> PfcInitializer::getResultsAsVector(NeedlePose true_pose) {
	...
	// Add number cand pts
	results.push_back(to_string(left_templ.params.num_cand_pts));
